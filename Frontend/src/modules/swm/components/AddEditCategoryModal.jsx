...
import { swmCategoryAddApi, swmCategoryDtlApi, swmCategoryEditApi } from '../../../api/endpoints';
...
function AddEditCategoryModal({ item, onClose, onSuccess }) {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        if (item?.id) {
          const response = await axios.post(swmCategoryDtlApi,{id:item?.id},{headers:{Authorization: `Bearer ${token}`,}});
    ...
    const handleFormSubmit = async () => {
      setIsFrozen(true);
      try {
        const url = item?.id ? swmCategoryEditApi : swmCategoryAddApi;
