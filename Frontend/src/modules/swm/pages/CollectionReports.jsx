...
function CollectionReports() {
  ...
  const summaryLink = useMemo(() => {
    ...
    return `/swm/report/payment/mode/summary?${params.toString()}`;
  }, [filters]);
  ...
  const fetchPaymentMode = async () => {
    try {
      const response = await axios.post(swmPaymentModeListApi);
  ...
  const fetchAllData = async () => {
    try {
      ...
      return (res.data.data?.data || []).map((item) => ({
        ...item,
        tranDate: formatLocalDate(item.tranDate,'-'),
        tranTime: formatTimeAMPM(item.createdAt),
      }));
    } catch (err) {
  ...
  const renderRow = (item, index, currentPage, perPage) => (
    <tr key={item.id} className="hover:bg-gray-50">
      ...
      <td className="px-3 py-2 border">{item.rate}</td>
      <td className="px-3 py-2 border whitespace-nowrap">{formatLocalDate(item.fromDate,'-') +` to ` + formatLocalDate(item.uptoDate,'-')}</td>
      <td className="px-3 py-2 border">{formatLocalDate(item.tranDate,'-')}</td>
      <td className="px-3 py-2 border">{formatTimeAMPM(item.createdAt)}</td>
      ...
      <td className="flex items-center gap-2 px-3 py-2 border">
        <Link
          to={`/swm/payment-receipt/${item.id}`}
          target="_blank"
  ...
  const renderFooter = (totals) => (
    <tr className="bg-gray-200 font-bold">
      ...
      <td colSpan={13} className="px-3 py-2 border border-white">{totals.totalCount}</td>
