...
function ConsumerDetails() {
  ...
  const[isVisitingLogOpen,setIsVisitingLogOpen] = useState(false);
  ...
  const actions = [
    ...
    },
    {
      ...
      show: appData?.userPermission?.canTakePayment ?? false,
    },
    {
      label: "Generate Demand",
      onClick: () => {
        setIsDemandGenerateModalOpen(true);
      },
      icon: <FaFortAwesome />,
      show: (appData?.userPermission?.canGenerateDemand ?? false),
    ...
    },
    {
      ...
      icon: <FaEdit />,
      show: appData?.userPermission?.canAppEdit ?? false,
    },
    {
      label: "Deactivate",
      onClick: () => {
      ...
      icon: <FaTrash />,
      show:(appData?.userPermission?.canAppLock ?? false) ,
      className: `bg-red-600 hover:bg-red-700`,
      },
    {
      label: "Visiting Log",
      onClick: () => {
        setIsVisitingLogOpen(true);
      },
      icon: <FaEdit />,
      show: true,
    },
        ...
        {appData ? (
          <>
            ...
            <SectionCard
              ...
              renderRow={(owner, idx) => (
                <tr key={idx}>
                  ...
                  <td className="px-3 py-2 border">{owner.dob}</td>
                </tr>
              )}
            />
            ...
            {isVisitingLogOpen &&(
              <VisitingLogModal
                id={id}
                onClose={() => setIsVisitingLogOpen(false)}
              />
            )}

            {/* Deactivate modal */}
            {isDeactivateModalOpen &&(
              <DeactivateConsumerModal
              id={id}
